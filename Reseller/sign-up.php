...
//Sign up validation and insert to database Reseller
...
if(isset($_POST['sign-up'])){
    ...
    $query = "SELECT * FROM reseller WHERE email='$e_mail'";
    ...
    if(empty($pass)){
        ...
        $_SESSION['error5'] = "Password must be at least 8 characters";
    }else {
        $password=$pass;
    }
        ...
        if($firstname==""  ||$lastname=="" || $contact=="" || $email=="" || $password==""){
            ...
            $sql = "INSERT INTO reseller SET firstname='$firstname', lastname='$lastname', contact='$contact', email='$email', password='$password', code='0', plan='regular', month='0', status='not verified', date='$date', is_verified='false', uid='$finalKey'";
            $query_insert = $conn->prepare( $sql );
		    $query_insert->execute();

                $query = "SELECT * FROM reseller WHERE email='$email'";
                $result = mysqli_query($conn, $query);
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        $sqla = "INSERT INTO `reseller_profile_pic` SET reseller_id='$id', filepath='$filepath', status='0'";
                                                          ...
                                                          <td align="center" style="padding:0;Margin:0;padding-top:10px;padding-bottom:10px">
                                                           <gdiv class="ginger-module-highlighter ginger-module-highlighter-float" style="background:none 0% 0% / auto repeat scroll padding-box border-box #FFFFFF;position:absolute;height:19.3333px;width:560px">
                                                            ...
                                                            <gdiv class="ginger-module-highlighter-float-content" style="height:96px">
                                                             <strong><a target="_blank" href="https://buildupph.com/confirm/reseller/?uid='.$finalKey.'" style="-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;text-decoration:underline;color:#2CB543;font-size:16px;height:96px">https://buildupph.com/confirm/reseller/?uid='.$finalKey.'</a></strong> 
                                                            </gdiv> 
                                                           </gdiv></td>
                                                         </tr>
                                                       </table></td>
                ...
                <div class="signup-pad">
                    <form action="sign-up.php" method="post" onkeydown="return event.key != 'Enter';">
                        <h2>Create Account</h2>
                            ...
                            <p id="terms" style="display:none">By clicking Sign Up, you agree to our <input type="button" value="Terms and Conditions" onclick="myadmin();" class="admin-btn" id="profile-btn"></p>
