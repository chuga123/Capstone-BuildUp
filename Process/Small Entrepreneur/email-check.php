...
error_reporting(E_ALL);
    ...
    if (isset($_POST['check'])) {
        ...
        $emailCheckQuery = "SELECT * FROM entrep WHERE email = '$email'";
        $emailCheckResult = mysqli_query($conn, $emailCheckQuery);

        if ($emailCheckResult) {
            if (mysqli_num_rows($emailCheckResult) > 0) {
                $code = rand(999999, 111111);
                $updateQuery = "UPDATE entrep SET code = $code WHERE email = '$email'";
                ...
                if ($updateResult) {
                    ...
                    header('location: ../../Small-Entrepreneur/forgot-password.php');
                }
            }else{
